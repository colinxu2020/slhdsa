...
def test2():
    assert FORS(shake_256s).node(b"", 0, shake_256s.a+1, b"", Address(0, 0)) == b""
    assert XMSS(shake_256s).node(b"", 0, shake_256s.h_m+1, b"", Address(0, 0)) == b""
    
def test3():
    with raises(ValueError):
        slhdsa.lowlevel.parameters.mgf1_sha256(b"", sha256().digest_size << 32)
    with raises(ValueError):
        slhdsa.lowlevel.parameters.mgf1_sha512(b"", sha512().digest_size << 32)
